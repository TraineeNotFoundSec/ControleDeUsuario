  ...
  if validador then
    begin
      ...
      if Panel2.Visible then
        ...
        btnRecuperar.Caption := 'Recuperar';
        btnNovoAcesso.Hint := 'Listar todos os usuários';
...
begin
  ...
  if Panel4.Visible then
    begin
      ...
      btnRecuperar.Hint := 'Fechar janela de recuperação';
    end

  else
    begin
      btnRecuperar.Caption := 'Recuperar';
      btnRecuperar.Hint := 'Recuperar acesso à conta';
    end;
...
var
  ...
  begin
    if (Edit4.Text = '') or (Edit6.Text = '') or (Edit7.Text = '') then
      begin
        ShowMessage('Preencha os campos Nome, Usuário e Senha para continuar');
      end
    else
      begin
        if (Edit2.Text = '') then
        begin
          resposta := MessageBox(Handle, 'Deseja continuar com a inserção deste usuário?', 'Confirmação', MB_YESNO + MB_ICONQUESTION);
            if Resposta = mrYes then
              begin
                ...
                if FDQuery8.Fields[0].AsInteger > 0 then
                  begin
                    ShowMessage('O usuário ' + Edit6.Text + ' já está em uso! Por gentileza, tente outro.');
                    Exit;
                  end
                else
                  begin
    ...
    begin
      if (Edit2.Text = '') then
        ...
        if(Edit2.Text <> '1') then
          begin
            resposta := MessageBox(Handle, 'Você tem certeza que deseja remover este usuário?', 'Confirmação', MB_YESNO + MB_ICONQUESTION);
            if Resposta = mrYes then
              begin
                FDQuery5.ExecSQL;
                FDQuery6.ExecSQL;
              ...
              begin
                ShowMessage('Remoção cancelada.');
              end;
          end

        else
          begin
            ShowMessage('Não é possível excluir a conta Master');
          end;
        end;
    end;
    ...
    begin
      ...
      if (FDQuery3.FieldByName('ID').AsString = '') then
        ...
        begin
          ...
          FDQuery5.SQL.Text := query4;
          FDQuery5.ExecSQL;
          // ShowMessage('Usuário adicionado ao grupo com sucesso!');
        end;
...
begin
  if (Edit2.Text = '') then
    begin
      ShowMessage('Selecione um usuário e tente novamente');
    end
  else
    begin
      query2 := 'SELECT g.id_group AS "ID", g.description AS "Nome" FROM groups g LEFT JOIN users_groups ug ON g.id_group = ug.id_group AND ug.id_user = ' + FDQuery2.FieldByName('ID').AsString + 'WHERE ug.id_group IS NULL;';
        ...
        begin
          ...
          begin
            if (FDQuery4.FieldByName('Nome').AsString = 'Administrator') then
              begin
                ShowMessage('Você não pode remover o usuário Master do grupo '+ (FDQuery4.FieldByName('Nome').AsString));
                Exit;
              end
            else
              begin
          ...
          // ShowMessage('Usuário removido do grupo com sucesso!');
        end;
